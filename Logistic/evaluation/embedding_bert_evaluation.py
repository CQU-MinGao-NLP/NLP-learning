...
RESULT_ROOT = "../Data/result/Bert_premodel_for_NLP/"

def test(text):
    text = en_tokenizer(text)
    words = Data_process.highfrequency(text, 0)[1]

    Bert2vec = torch.load(MODEL_ROOT + 'Bert_premodel_for_NLP.pkl')
    token_to_idx = Data_process.dict_load(MODEL_ROOT + "Bert_token2idx.txt")
    idx_to_token = Data_process.dict_load(MODEL_ROOT + "Bert_idx2token.txt")
    ...
    with open(RESULT_ROOT + "word_embedding.txt", 'w', encoding='UTF-8') as f:
        ...
        for word in words:
            ...
            word_emb = get_embedding(word, Bert2vec.embedding.tok_embed)\
